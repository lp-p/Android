package com.example.coverflow.general;
...
public abstract class ToolBox {
	...
	public static float getVectorLength(PointF v){
		...
		float sqrt;
		sqrt = (float) sqrt(v.x*v.x + v.y*v.y);
		return sqrt;
	}
	...
	public static int roundToInt(float num){
		if((num - floor(num)) > 0.5f){ //round up
			return (int) ceil(num);
		}
		else{ //round down
    ...
    public static PointF getCircleIntersection(PointF center,float radius, PointF touchPoint){
		...
		final float dr = (float) sqrt(dx*dx + dy*dy);
    	...
    	res1.y = (abs(dy)*r*dr)/(dr*dr);
    	res2.y = (-1 * abs(dy)*r*dr)/(dr*dr);
...
//   }
    ...
    public static String humanReadableByteCount(long bytes, boolean si) {
        ...
        return String.format(Locale.US,"%.1f %sB", bytes / pow(unit, exp), pre);
    }
    ...
    public static class ViewCache <T extends View> {
		...
		public void cacheView(T v){
			WeakReference<T> ref = new WeakReference<>(v);
    ...
    public static byte[] IMEItobyteConverter(String IMEI) {
	    ...
	    byte[] data;
		data = ByteBuffer.allocate(8).putLong(imeiInLong).array();
		return data;
    }
